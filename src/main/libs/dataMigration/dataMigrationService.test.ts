...
const makeTempDir = (): string => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'heyclaw-data-migration-test-'));
  tempRoots.push(dir);
  return dir;
};
...
const writeManifestFixture = (userDataPath: string): void => {
  writeFile(
    path.join(userDataPath, '.heyclaw-migration.json'),
    JSON.stringify({
      format: 'heyclaw-data-migration',
      version: 1,
      archiveRoot: 'HeyClaw',
      sqlite: { exists: true },
...
test('createMigrationArchive excludes cache and log data and writes a manifest', () => {
  ...
  const userData = path.join(root, 'HeyClaw');
  ...
  writeFile(path.join(userData, 'backups', 'sqlite', 'snapshots', 'heyclaw-latest.sqlite'), 'old-snapshot');
  writeFile(path.join(userData, 'sqlite-backups', 'heyclaw-latest.sqlite'), 'legacy-snapshot');
  ...
  expect(entries).toContain('HeyClaw/.heyclaw-migration.json');
  expect(entries).toContain(`HeyClaw/${DB_FILENAME}`);
  expect(entries).toContain('HeyClaw/openclaw/state/openclaw.json');
  expect(entries).toContain('HeyClaw/SKILLs/demo/SKILL.md');
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(extractRoot, 'HeyClaw', '.heyclaw-migration.json'), 'utf8'),
  ) as {
    format?: string;
    archiveRoot?: string;
  ...
  expect(manifest.format).toBe('heyclaw-data-migration');
  expect(manifest.archiveRoot).toBe('HeyClaw');
...
test('createMigrationArchive replaces the live sqlite database with the snapshot', () => {
  ...
  const extractRoot = extractArchive(archivePath);
  expect(readSqliteString(path.join(extractRoot, 'HeyClaw', DB_FILENAME), 'SELECT value FROM kv WHERE key = ?', ['auth_tokens']))
    .toContain('snapshot-refresh');
  expect(fs.existsSync(path.join(extractRoot, 'HeyClaw', `${DB_FILENAME}-wal`))).toBe(false);
});
...
test('inspectMigrationArchive rejects legacy Windows PowerShell archive root', () => {
  const root = makeTempDir();
  const legacyRoot = path.join(root, 'AppData', 'Roaming', 'HeyClaw');
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(/does not contain HeyClaw user data/);
});

test('inspectMigrationArchive rejects archives without a migration manifest', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'HeyClaw');
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(/missing \.heyclaw-migration\.json/);
});
...
test('inspectMigrationArchive rejects archives whose manifest does not match sqlite content', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', 'HeyClaw');
  ...
  const extractRoot = extractArchive(archivePath);
  const manifestPath = path.join(extractRoot, 'HeyClaw', '.heyclaw-migration.json');
...
test('performPendingDataMigrationRestoreSync creates rollback and restores backup data', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', 'HeyClaw');
  const targetUserData = path.join(root, 'target', 'HeyClaw');
...
test('performPendingDataMigrationRestoreSync replaces data in place and preserves runtime locks', () => {
  ...
  writeFile(path.join(sourceUserData, 'backups', 'sqlite', 'snapshots', 'heyclaw-latest.sqlite'), 'source-backup');
  writeFile(path.join(sourceUserData, 'sqlite-backups', 'heyclaw-latest.sqlite'), 'source-legacy-backup');
  ...
  writeFile(path.join(targetUserData, 'old-only.txt'), 'old');
  writeFile(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', 'heyclaw-latest.sqlite'), 'target-backup');
  writeFile(path.join(targetUserData, 'sqlite-backups', 'heyclaw-latest.sqlite'), 'target-legacy-backup');
  ...
  createMigrationArchiveSync({ userDataPath: sourceUserData, outputPath: archivePath });
  const extractRoot = extractArchive(archivePath);
  writeFile(path.join(extractRoot, 'HeyClaw', 'backups', 'sqlite', 'snapshots', 'heyclaw-latest.sqlite'), 'legacy-source-backup');
  writeFile(path.join(extractRoot, 'HeyClaw', 'sqlite-backups', 'heyclaw-latest.sqlite'), 'legacy-source-legacy-backup');
  writeFile(path.join(extractRoot, 'HeyClaw', 'cowork', 'bin', 'node.cmd'), 'legacy-source-shim');
  writeFile(path.join(extractRoot, 'HeyClaw', 'install-timing.log'), 'legacy-source-install-log');
  writeFile(path.join(extractRoot, 'HeyClaw', 'skill-migrate.log'), 'legacy-source-skill-migrate-log');
  writeFile(path.join(extractRoot, 'HeyClaw', 'Dictionaries', 'source.bdic'), 'legacy-source-dictionary');
  writeFile(path.join(extractRoot, 'HeyClaw', 'Local Storage', 'leveldb', 'source.log'), 'legacy-source-local-storage');
  writeFile(path.join(extractRoot, 'HeyClaw', 'Preferences'), 'legacy-source-preferences');
  writeFile(path.join(extractRoot, 'HeyClaw', 'Session Storage', 'leveldb', 'source.log'), 'legacy-source-session-storage');
  writeFile(path.join(extractRoot, 'HeyClaw', 'openclaw', 'logs', 'gateway-2026-06-10.log'), 'legacy-source-gateway-log');
  writeFile(path.join(extractRoot, 'HeyClaw', 'openclaw', 'mcp-packages', 'demo', 'node_modules', 'native.node'), 'legacy-source-native');
  writeFile(path.join(extractRoot, 'HeyClaw', 'openclaw', 'state', 'logs', 'commands.log'), 'legacy-source-commands-log');
  writeFile(path.join(extractRoot, 'HeyClaw', 'runtimes', 'python', 'python.exe'), 'legacy-source-runtime');
  tar.create({
    sync: true,
    gzip: true,
    file: archivePath,
  ...
  expect(fs.existsSync(path.join(targetUserData, `${DB_FILENAME}-shm`))).toBe(false);
  expect(fs.existsSync(path.join(targetUserData, 'old-only.txt'))).toBe(false);
  expect(fs.readFileSync(path.join(targetUserData, 'backups', 'sqlite', 'snapshots', 'heyclaw-latest.sqlite'), 'utf8'))
    .toBe('target-backup');
  expect(fs.readFileSync(path.join(targetUserData, 'sqlite-backups', 'heyclaw-latest.sqlite'), 'utf8'))
