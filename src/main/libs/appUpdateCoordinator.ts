...
const APP_UPDATE_TEST_CURRENT_VERSION_ENV = 'HEYCLAW_UPDATE_CURRENT_VERSION';
...
export class AppUpdateCoordinator {
  ...
  private isCachedInstallerForSource(filename: string, source: AppUpdateSource | null): boolean {
    if (!filename.startsWith('heyclaw-update-')) {
      return false;
    }
    if (source == null) {
      return true;
    }
    if (filename.startsWith(`heyclaw-update-${source}-`)) {
      return true;
    }
    return /^heyclaw-update-\d+/.test(filename);
  }
