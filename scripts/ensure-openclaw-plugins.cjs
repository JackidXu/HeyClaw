...
function findInstalledPluginDir(stagingDir, plugin) {
  ...
  const result = selectInstalledPluginDir(npmCandidates, plugin);
  if (!result) {
    console.log(`[DEBUG] findInstalledPluginDir failed for plugin ${plugin.id}.`);
    console.log(`[DEBUG] npmProjectsDir exists: ${fs.existsSync(npmProjectsDir)}`);
    if (fs.existsSync(npmProjectsDir)) {
      const projects = fs.readdirSync(npmProjectsDir);
      console.log(`[DEBUG] Projects in npm/projects: ${JSON.stringify(projects)}`);
      for (const proj of projects) {
        const nmDir = path.join(npmProjectsDir, proj, 'node_modules');
        console.log(`[DEBUG] node_modules exists for ${proj}: ${fs.existsSync(nmDir)}`);
        if (fs.existsSync(nmDir)) {
          console.log(`[DEBUG] contents of node_modules: ${JSON.stringify(fs.readdirSync(nmDir))}`);
          const scopedDirs = fs.readdirSync(nmDir).filter(f => f.startsWith('@'));
          for (const s of scopedDirs) {
            console.log(`[DEBUG] contents of scoped dir ${s}: ${JSON.stringify(fs.readdirSync(path.join(nmDir, s)))}`);
          }
        }
      }
    }
