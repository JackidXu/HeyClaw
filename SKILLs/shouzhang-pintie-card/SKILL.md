---
name: shouzhang-pintie-card
description: 生成适配Seedream/Doubao系列图像模型的JSON提示词，把单张主体照片做成小红书/INS风格手帐拼贴卡（手账/台账/剪贴簿）——原图主体100%保留，裁剪3-5个图中真实存在的子元素做白边贴纸效果，叠加和纸胶带、网格纸背景、中文手写标注、氛围匹配装饰。运行三核流水线（分析→风格匹配→编译），输出可视化分析报告+可直接执行的JSON。使用场景：制作小红书/INS手帐拼贴卡、手账风格穿搭/美食/玩具/宠物/好物/打卡排版、将主体照片转换为装饰拼贴提示词。触发词：手帐拼贴卡、拼贴卡生成、台账拼贴、拼贴手账、剪贴簿、scrapbook、小红书台账、INS拼贴、手账风、把这张图做成拼贴/手账、穿搭拼贴、美食拼贴、好物拼贴。注意：本技能指视觉拼贴类台账，非财务记账类台账。优先使用Seedream（Doubao-Seedream-5.0-lite）图像模型生成最终结果。
version: 1.0.0
---
...
4. **Phase 4 生成图片** → 出 JSON 后，**优先使用Seedream（Doubao-Seedream-5.0-lite）模型**调用当前环境可用的文生图工具生成拼贴卡图；无Seedream时兼容其他图像模型；当前环境无图像生成能力时，明确告知并交回提示词供用户自行使用。
...
**Part 2 — Seedream/Doubao系列图像模型提示词（JSON 代码块）**，须为可直接执行的纯结构，块内禁注释：
